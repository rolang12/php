<?php
	...
	if (isset($_SESSION['session']) && $_SESSION['session'] == 'active') {
		...
		if (isset($_GET['idusuarios'])){
			$id = $_REQUEST['idusuarios'];
			$pais = mysqli_query($conexion, "SELECT * FROM usuarios INNER JOIN paises ON paises.id=usuarios.paises_id where paises.id=usuarios.paises_id and usuarios.idusuarios=$id ");

			$name = mysqli_query($conexion, "SELECT nickname,avatar from usuarios where idusuarios = $id");
			$sql = "SELECT avatar,usuarios.nickname as nickname, articulos.titulo as titulo, articulos.contenido as contenido FROM usuarios INNER JOIN articulos ON articulos.usuarios_idusuarios=usuarios.idusuarios where articulos.usuarios_idusuarios=$id";
			$sql2 = "SELECT count(articulos.titulo) as 'cantidad' FROM usuarios INNER JOIN articulos ON articulos.usuarios_idusuarios=usuarios.idusuarios where articulos.usuarios_idusuarios=$id";
			...
			$can=mysqli_fetch_array($result);
			$name=mysqli_fetch_array($name);

			$i=mysqli_fetch_assoc($result2);
			$paise=mysqli_fetch_array($pais);
		}	
...
<?php while ($i1=mysqli_fetch_array($result2))
{
	echo "Nombre: ".$i1['nickname'];	
	echo "Titulo: ".$i1['titulo'];
	echo "<br> <br>";
	echo "Contenido: ".$i1['contenido'];
	echo "Avatar: ".$i1['avatar'];
}
?>
...
<body class="font-sans antialiased text-gray-900 leading-normal tracking-wider bg-cover" style="background-image:url('https://source.unsplash.com/1L71sPT5XKc');">
  ...
  <div class="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto my-32 lg:my-0">
	...
	<div class="w-full lg:w-2/5">
		...
		<img src="<?php echo $name['avatar']  ?> " class="rounded-none lg:rounded-lg shadow-2xl hidden lg:block">
